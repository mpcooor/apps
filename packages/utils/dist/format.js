export function bufferToHex(array) {
...
export function keyShareToString(key) {
    for (let i = 0; i < key.localKey.h1_h2_n_tilde_vec.length; i++) {
        key.localKey.h1_h2_n_tilde_vec[i].N = bufferToHex(key.localKey.h1_h2_n_tilde_vec[i].N);
        key.localKey.h1_h2_n_tilde_vec[i].g = bufferToHex(key.localKey.h1_h2_n_tilde_vec[i].g);
        key.localKey.h1_h2_n_tilde_vec[i].ni = bufferToHex(key.localKey.h1_h2_n_tilde_vec[i].ni);
    }
    key.localKey.keys_linear.x_i.scalar = bufferToHex(key.localKey.keys_linear.x_i.scalar);
    key.localKey.keys_linear.y.point = bufferToHex(key.localKey.keys_linear.y.point);
    for (let i = 0; i < key.localKey.pk_vec.length; i++) {
        key.localKey.pk_vec[i].point = bufferToHex(key.localKey.pk_vec[i].point);
    }
    for (let i = 0; i < key.localKey.vss_scheme.commitments.length; i++) {
        key.localKey.vss_scheme.commitments[i].point = bufferToHex(key.localKey.vss_scheme.commitments[i].point);
    }
    key.localKey.y_sum_s.point = bufferToHex(key.localKey.y_sum_s.point);
    return JSON.stringify(key);
}
export function keyShareFromString(rawKey) {
...
export function presignStateToString(presign) {
    presign.presign.R.point = bufferToHex(presign.presign.R.point);
    for (let i = 0; i < presign.presign.local_key.h1_h2_n_tilde_vec.length; i++) {
        presign.presign.local_key.h1_h2_n_tilde_vec[i].N = bufferToHex(presign.presign.local_key.h1_h2_n_tilde_vec[i].N);
        presign.presign.local_key.h1_h2_n_tilde_vec[i].g = bufferToHex(presign.presign.local_key.h1_h2_n_tilde_vec[i].g);
        presign.presign.local_key.h1_h2_n_tilde_vec[i].ni = bufferToHex(presign.presign.local_key.h1_h2_n_tilde_vec[i].ni);
    }
    presign.presign.local_key.keys_linear.x_i.scalar = bufferToHex(presign.presign.local_key.keys_linear.x_i.scalar);
    presign.presign.local_key.keys_linear.y.point = bufferToHex(presign.presign.local_key.keys_linear.y.point);
    for (let i = 0; i < presign.presign.local_key.pk_vec.length; i++) {
        presign.presign.local_key.pk_vec[i].point = bufferToHex(presign.presign.local_key.pk_vec[i].point);
    }
    for (let i = 0; i < presign.presign.local_key.vss_scheme.commitments.length; i++) {
        presign.presign.local_key.vss_scheme.commitments[i].point = bufferToHex(presign.presign.local_key.vss_scheme.commitments[i].point);
    }
    presign.presign.local_key.y_sum_s.point = bufferToHex(presign.presign.local_key.y_sum_s.point);
    presign.presign.sigma_i.scalar = bufferToHex(presign.presign.sigma_i.scalar);
    presign.presign.sign_keys.g_gamma_i.point = bufferToHex(presign.presign.sign_keys.g_gamma_i.point);
    presign.presign.sign_keys.g_w_i.point = bufferToHex(presign.presign.sign_keys.g_w_i.point);
    presign.presign.sign_keys.gamma_i.scalar = bufferToHex(presign.presign.sign_keys.gamma_i.scalar);
    presign.presign.sign_keys.k_i.scalar = bufferToHex(presign.presign.sign_keys.k_i.scalar);
    presign.presign.sign_keys.w_i.scalar = bufferToHex(presign.presign.sign_keys.w_i.scalar);
    for (let i = 0; i < presign.presign.t_vec.length; i++) {
        presign.presign.t_vec[i].point = bufferToHex(presign.presign.t_vec[i].point);
    }
    return JSON.stringify(presign);
}
export function presignStateFromString(rawPresign) {
    ...
    for (let i = 0; i < presign.presign.local_key.vss_scheme.commitments.length; i++) {
        presign.presign.local_key.vss_scheme.commitments[i].point = hexToBuffer(presign.presign.local_key.vss_scheme.commitments[i].point);
    }
...
export function roundKeygenToString(roundKeygen) {
...
function round1KeygenToString(roundKeygen) {
    roundKeygen.body.Round1.com = bufferToHex(roundKeygen.body.Round1.com);
    roundKeygen.body.Round1.composite_dlog_proof_base_h1.x = bufferToHex(roundKeygen.body.Round1.composite_dlog_proof_base_h1.x);
    roundKeygen.body.Round1.composite_dlog_proof_base_h1.y = bufferToHex(roundKeygen.body.Round1.composite_dlog_proof_base_h1.y);
    roundKeygen.body.Round1.composite_dlog_proof_base_h2.x = bufferToHex(roundKeygen.body.Round1.composite_dlog_proof_base_h2.x);
    roundKeygen.body.Round1.composite_dlog_proof_base_h2.y = bufferToHex(roundKeygen.body.Round1.composite_dlog_proof_base_h2.y);
...
function round2KeygenToString(roundKeygen) {
...
function round3KeygenToString(roundKeygen) {
...
function round4KeygenToString(roundKeygen) {
...
export function roundKeygenFromString(roundKeygen) {
...
function round1KeygenFromString(parsed) {
...
function round2KeygenFromString(parsed) {
...
function round3KeygenFromString(parsed) {
...
function round4KeygenFromString(parsed) {
