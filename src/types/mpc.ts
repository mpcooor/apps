...
export interface IMessage {
  // biome-ignore lint/suspicious/noExplicitAny: messages can have multiple types
  body: any
  receiver?: number
  round: number
  sender?: number
...
export interface ILocalKey {
  h1_h2_n_tilde_vec: { N: Uint8Array | string; g: Uint8Array | string; ni: Uint8Array | string }[]
  i: number
  keys_linear: { x_i: ICurveScalar; y: ICurvePoint }
  n: number
  paillier_dk: { p: string; q: string }
  paillier_key_vec: { n: string }[]
  pk_vec: ICurvePoint[]
  t: number
  vss_scheme: {
