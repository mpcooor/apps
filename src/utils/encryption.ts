...
async function generateSalt(): Promise<string | undefined> {
  try {
...
export async function encryptVault(
  vault: string,
  passwordOrSaltedKey: string | ISaltedKey
): Promise<IEncryptedVault | undefined> {
  try {
    const { crypto } = global
...
export async function decryptVault(
  ...
  try {
    ...
    const plaintext = await crypto.subtle.decrypt(
      { iv: hexToBuffer(initializationVector), name: 'AES-GCM' },
      key,
